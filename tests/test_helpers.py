...
def test_validate_gt_int_enforces_lower_bound():
    ...
    assert validate_gt_int(10, 5) == 10, (
...
def test_get_semaphore_caps_concurrency():
    ...
    assert get_semaphore(2)._value == 2, (
        "Requested concurrency below the cap should be preserved."
    )
    assert get_semaphore(CONCURRENCY_LIMIT + 20)._value == CONCURRENCY_LIMIT, (
...
def test_atomic_write_helpers_create_expected_files(tmp_path):
    ...
    assert json_path.read_text() == '{"a": 1, "b": [2, 3]}', (
        "JSON writes should preserve the original data structure."
    )
    assert bytes_path.read_bytes() == b"abc123", (
        "Binary writes should preserve the original byte content."
    )
    assert not json_path.with_suffix(".json.tmp").exists(), (
        "Temporary JSON files should be cleaned up after the atomic write."
    )
    assert not bytes_path.with_suffix(".bin.tmp").exists(), (
        "Temporary binary files should be cleaned up after the atomic write."
    )
...
def test_parse_doc_helpers_handle_numpy_and_google_styles():
    ...
    assert sections["title"] == "My Function", (
        "The first non-empty line should be treated as the section title."
    )
    assert sections["description"] == "This function does something.", (
    ...
    assert is_numpy_header(["Parameters", "----------"], 0) is True, (
        "NumPy-style section headers should be detected."
    )
    assert is_numpy_header(["Notes", "-----"], 0) is True, (
        "Known section names with dash underlines should be detected."
    )
    assert is_numpy_header(["Other", "-----"], 0) is False, (
...
def test_get_docstring_uses_callable_docstring_first():
    ...
    assert get_docstring(module, "sample_function") == "Sample callable docstring.", (
