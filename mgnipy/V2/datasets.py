...
class MGazine(MGnifyAnalysisWithAnnotations):
    ...
    def __init__(self, accession: str):
        self._mgnifier_helper = MGnifier()
        super().__init__(
            # **data,
        )
    ...
    def list_downloads_files(self):
        ...
        return [f.url for f in self.downloads]
    ...
    def stream_tsv(
        ...
        """
        Streams a tsv file from a url and returns an iterator of pandas dataframes.
        ...
        chunksize : int, optional
            The number of rows to include in each chunk. Default is 1000.
    ...
    def stream_txt(self, url: str, **httpx_kwargs) -> Generator:
        """
        ...
        """
        with self._mgnifier_helper.httpx_client.stream(
            "GET", url, **httpx_kwargs
        ) as response:
    ...
    async def adownload(
        ...
        *,
        ...
        async with semaphore:
            # reuse httpx async client if provided, otherwise init new client using mgnifier
            async with httpx_aclient or self._mgnifier_helper.httpx_aclient as client:
                async with client.stream("GET", _url) as response:
                    ...
                    async with (
                        aiofiles.open(filepath, "wb") as f,
                        tqdm(
                            total=total,
                            unit="B",
                            unit_scale=True,
                            desc=str(filename or _alias),
                        ) as pbar,
                    ):
    ...
    async def adownload_all(
        ...
        async with self._mgnifier_helper.httpx_aclient as client:
            ...
            tasks = [
                self.adownload(to_dir=to_dir, alias=a, httpx_aclient=client)
                for a in self.downloads_dict
            ]
            # Overall progress bar
            for f in tqdm(
                asyncio.as_completed(tasks), total=len(tasks), desc="Overall Progress"
            ):
                await f
